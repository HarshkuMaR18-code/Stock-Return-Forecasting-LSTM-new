...
from src.constants import FEATURE_COLUMNS, FORECAST_HORIZON_DAYS, SEQUENCE_WINDOW_SIZE, TARGET_COLUMN
...
class DataTransformation:
    ...
    def add_technical_features(group: pd.DataFrame) -> pd.DataFrame:
        group = group.copy()
        group["return_1d"] = group["Close"].pct_change(fill_method=None)
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        feature_dfs = [self.add_technical_features(group) for _, group in df.groupby("ticker", sort=False)]
        if feature_dfs:
    ...
    def run(self) -> tuple[str, str, str]:
        try:
            ...
            os.makedirs(os.path.dirname(TRAIN_NPZ_PATH), exist_ok=True)
